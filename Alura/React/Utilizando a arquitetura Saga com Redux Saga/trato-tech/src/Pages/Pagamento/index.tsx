...
export default function Pagamento() {
  ...
  const mudarFormaDePagamento = (e: React.ChangeEvent<HTMLSelectElement>) => {
    if (e.target.value === '-')
      return setFormaDePagamento(formaPagamentoInicial);
    const cartaoSelecionado = usuario.cartoes?.find(
      cartao => cartao.id === e.target.value
    );
        ...
        <div className={styles.content}>
          {formaDePagamento.id !== '-' && (
            <>
              ...
              <p>
                O saldo deste cartão é de {valorMoeda(formaDePagamento.saldo)}
              </p>
            </>
          )}
          <p>Total com taxas: {valorMoeda(total * formaDePagamento.taxa)}</p>
        </div>
